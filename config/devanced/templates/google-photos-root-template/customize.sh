...
ui_print "- Package mode: Google Photos original package"
ui_print "- Features: System App Mount + Zygisk Pixel Spoofing"
ui_print ""

# Zygisk Check
if [ "$MAGISK_VER_CODE" -lt 24000 ] && [ ! -f /data/adb/ksu/bin/busybox ] && [ ! -f /data/adb/apatch/bin/busybox ]; then
  ui_print "⚠️ Warning: This module requires Zygisk."
  ui_print "Please ensure Zygisk is enabled in your root manager settings."
  ui_print ""
fi

# Clean up unused architectures for Zygisk libs
if [ -d "$MODPATH/zygisk" ]; then
  ui_print "- Device architecture: $ARCH"
  if [ "$ARCH" = "arm" ] || [ "$ARCH" = "arm64" ]; then
    rm -rf "$MODPATH/zygisk/x86" "$MODPATH/zygisk/x86_64"
    if [ "$ARCH" = "arm" ]; then
    ...
    rm -rf "$MODPATH/zygisk/armeabi-v7a" "$MODPATH/zygisk/arm64-v8a"
    if [ "$ARCH" = "x86" ]; then
    ...
    ui_print "⚠️ Warning: Unknown architecture $ARCH. Keeping all Zygisk libs."
  fi
  
  # Set permissions for Zygisk libraries
...
ui_print "  After reboot, you MUST clear the storage/data of"
ui_print "  the Google Photos app for Pixel spoofing to take effect!"
ui_print ""
