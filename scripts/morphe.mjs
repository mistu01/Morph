...
function rootServiceScript(apps) {
  ...
  return [
    ...
    "}",
    "",
    "# Create per-app log directory under Downloads",
    "setup_dl_log() {",
    "  local label=\"$1\"",
    "  local base",
    "  base=\"$(find_download_dir)\" || { DL_LOG_DIR=\"\"; return 1; }",
    "  DL_LOG_DIR=\"$base/${label} Log\"",
    "  if ! mkdir -p \"$DL_LOG_DIR\" 2>/dev/null; then",
    "    log \"warn\" \"Could not create Download log dir: $DL_LOG_DIR\"",
    "    DL_LOG_DIR=\"\"",
    "    return 1",
    "  fi",
    "  # Copy the post-mount log (written before storage was available)",
    "  [ -f \"$LOG\" ] && cat \"$LOG\" >> \"$DL_LOG_DIR/module.log\" 2>/dev/null || true",
    "  log \"ok\" \"Download log: $DL_LOG_DIR\"",
    "}",
    "",
    ...
    "}",
    "",
    "# === Main: apply bind mounts ===",
    "while IFS='|' read -r pkg label fallback_path; do",
    "  [ -n \"$pkg\" ] || continue",
    "  # Set up Download log directory before apply so log() dual-writes",
    "  setup_dl_log \"$label\"",
    ...
    "  [ -n \"$pkg\" ] || continue",
    "  setup_dl_log \"$label\"",
    "  if [ -n \"$DL_LOG_DIR\" ]; then",
    "    log \"ok\" \"Starting logcat monitor for $label -> $DL_LOG_DIR/logcat.log\"",
    "    ( monitor_app_logcat \"$pkg\" \"$label\" \"$DL_LOG_DIR\" ) &",
    "  fi",
    "done <<EOF_MONITOR_APPS",
    "$APP_LIST",
    "EOF_MONITOR_APPS",
    "",
  ].join("\n");
