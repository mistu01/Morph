...
function rootCustomizeScript(apps) {
  ...
  return [
    ...
    "}",
    "",
    "install_stock_session_inner() {",
    "  local pkg=\"$1\" label=\"$2\" stock_dir=\"$3\" inherit=\"$4\"",
    "  local total size session out apk name create_args",
    ...
    "  create_args=\"--user 0 -i com.android.vending -r -d --dont-kill -S $total\"",
    "  if [ \"$inherit\" = \"1\" ]; then",
    "    create_args=\"--user 0 -i com.android.vending -r -d --dont-kill -p $pkg -S $total\"",
    "    ui_print \"  Inheriting existing package splits\"",
    "  fi",
    "  out=\"$(pm install-create $create_args 2>&1)\" || { ui_print \"$out\"; return 1; }",
    "  session=\"${out#*[}\"",
    "  session=\"${session%]*}\"",
    "  for apk in \"$stock_dir\"/*.apk; do",
    "    [ -f \"$apk\" ] || continue",
    "    size=\"$(wc -c < \"$apk\")\"",
    "    name=\"${apk##*/}\"",
    "    out=\"$(pm install-write -S \"$size\" \"$session\" \"$name\" \"$apk\" 2>&1)\" || { pm install-abandon \"$session\" >/dev/null 2>&1 || true; ui_print \"$out\"; return 1; }",
    "  done",
    "  out=\"$(pm install-commit \"$session\" 2>&1)\" || { ui_print \"$out\"; return 1; }",
    "}",
    "",
    "install_stock_session() {",
    "  local pkg=\"$1\" label=\"$2\" stock_dir=\"$3\"",
    "  local verify_adb package_verifier result",
    ...
    "  result=0",
    "  if ! install_stock_session_inner \"$pkg\" \"$label\" \"$stock_dir\" 0; then",
    "    ui_print \"  First install attempt failed, trying with split inheritance\"",
    "    if ! install_stock_session_inner \"$pkg\" \"$label\" \"$stock_dir\" 1; then",
    "      result=1",
    "    fi",
    "  fi",
    ...
    "  if [ \"$pkg\" = \"com.google.android.apps.photos\" ]; then",
    "    ui_print \"  Uninstalling original system Photos app for user 0 first...\"",
