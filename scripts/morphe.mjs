...
function createRootModule(moduleDir, { id, name, version, versionCode, description, apps }) {
  ...
  writeFileSync(join(moduleDir, "README.md"), [
    ...
    "During installation, the module removes any Play Store update overlay, re-enables the system package for user 0, then bind-mounts the patched APK over the system base APK path.",
    "The module re-applies the bind mount and Play Store detach commands at boot.",
    "This keeps the launcher entry tied to the original system package while running the patched APK.",
    "",
    "Included apps:",
...
function rootCustomizeScript(apps) {
  ...
  return [
    ...
    "ui_print \"- Install mode: system app bind mount (no APK re-registration)\"",
    ...
    "find_system_apk() {",
    "  local pkg=\"$1\"",
    "  local dumpsys_out path",
    "  dumpsys_out=\"$(dumpsys package \"$pkg\" 2>/dev/null)\"",
    "  path=\"$(printf '%s\\n' \"$dumpsys_out\" | grep -E -m1 'originalCodePath=' | sed -E 's/^[[:space:]]*originalCodePath=//; s/[[:space:]]*$//')\"",
    "  if [ -z \"$path\" ] || ! printf '%s\\n' \"$path\" | grep -E -q '^/(system|product|system_ext|vendor)/'; then",
    "    path=\"$(printf '%s\\n' \"$dumpsys_out\" | grep -E -m1 'codePath=' | sed -E 's/^[[:space:]]*codePath=//; s/[[:space:]]*$//')\"",
    "  fi",
    "  if [ -n \"$path\" ] && printf '%s\\n' \"$path\" | grep -E -q '^/(system|product|system_ext|vendor)/'; then",
    "    if [ -f \"$path\" ]; then",
    "      echo \"$path\"",
    ...
    "    elif [ -d \"$path\" ]; then",
    "      local apk",
    "      for apk in \"$path\"/*.apk \"$path\"/*/*.apk; do",
    "        [ -f \"$apk\" ] && { echo \"$apk\"; return 0; }",
    "      done",
    "    fi",
    "  fi",
    "  local dir parent name apk",
    "  for dir in /system/product/priv-app /system/product/app /system/priv-app /system/app /product/priv-app /product/app /system/system_ext/priv-app /system/system_ext/app; do",
    "    [ -d \"$dir\" ] || continue",
    ...
    "  [ -n \"$pkg\" ] || continue",
    "  install_root_apk \"$pkg\" \"$label\" \"$patched_name\" \"$fallback_path\"",
...
function rootServiceScript(apps) {
  ...
  return [
    ...
    ")",
    "",
    "# Dual-write to both $MODDIR log and Download folder log (if set up)",
    "log() {",
    "  local msg",
    "  msg=\"$(date '+%Y-%m-%d %H:%M:%S') [$1] $2\"",
    "  echo \"$msg\" >> \"$LOG\"",
    "  [ -n \"$DL_LOG_DIR\" ] && echo \"$msg\" >> \"$DL_LOG_DIR/module.log\" 2>/dev/null || true",
    "}",
    "",
    "# Find the first writable Download directory on the device",
    "find_download_dir() {",
    "  local candidate",
    "  for candidate in \"/sdcard/Download\" \"/storage/emulated/0/Download\" \"/data/media/0/Download\" \"/sdcard\"; do",
    ...
    "}",
    "",
    "# Writable storage helper",
    "is_storage_writable() {",
    "  local base=\"$1\"",
    "  [ -d \"$base\" ] && touch \"$base/.test_write\" 2>/dev/null && { rm -f \"$base/.test_write\" 2>/dev/null; return 0; }",
    "  return 1",
    "}",
    "",
    "# Start background logcat capture for a package, running until the app is detected",
    "# or until MAX_WAIT seconds elapse, then 30s more after app starts",
    "monitor_app_logcat() {",
    "  local pkg=\"$1\" label=\"$2\" log_dir=\"$3\"",
    "  local logcat_file=\"$log_dir/logcat.log\"",
    "  local MAX_WAIT=300",
    "  {",
    "    echo \"================================================\"",
    "    echo \" $label Logcat Capture\"",
    "    echo \" Package : $pkg\"",
    "    echo \" Started : $(date)\"",
    "    echo \"================================================\"",
    "    echo \"\"",
    "    echo \"--- Early boot buffer (logcat -d) ---\"",
    "    logcat -d -v time 2>&1 | grep -E \"($pkg|FATAL EXCEPTION|AndroidRuntime|force.clos|dalvikvm|ART |PackageManager|ActivityManager)\" || true",
    "    echo \"\"",
    "    echo \"--- Live logcat (filtered) ---\"",
    "  } >> \"$logcat_file\"",
    "  # Start live logcat in background",
    "  logcat -v time 2>&1 | grep --line-buffered -E \"($pkg|FATAL EXCEPTION|AndroidRuntime|force.clos|ART |PackageManager.*($pkg|Photos)|ActivityManager.*($pkg|Photos)|mount.*Photos)\" >> \"$logcat_file\" &",
    "  local logcat_pid=$!",
    "  # Monitor until app process is running",
    "  local waited=0",
    "  local app_running=0",
    "  while [ $waited -lt $MAX_WAIT ]; do",
    "    sleep 2",
    "    waited=$((waited + 2))",
    "    if ps -A 2>/dev/null | grep -qE \"[0-9] $pkg$\" || ps 2>/dev/null | grep -v grep | grep -qF \"$pkg\"; then",
    "      echo \"\" >> \"$logcat_file\"",
    "      echo \"=== $label PROCESS DETECTED RUNNING at $(date) ===\" >> \"$logcat_file\"",
    "      app_running=1",
    "      break",
    "    fi",
    "  done",
    "  if [ $app_running -eq 1 ]; then",
    "    # Capture 30s more after the app is running to see any post-launch errors",
    "    sleep 30",
    "    echo \"=== 30s post-launch capture complete at $(date) ===\" >> \"$logcat_file\"",
    "  else",
    "    echo \"\" >> \"$logcat_file\"",
    "    echo \"=== Timeout: $label process not detected after ${MAX_WAIT}s ===\" >> \"$logcat_file\"",
    "  fi",
    "  kill $logcat_pid 2>/dev/null || true",
    "  echo \"=== Capture ended: $(date) ===\" >> \"$logcat_file\"",
    "  log \"ok\" \"Logcat saved: $logcat_file\"",
    "}",
    "",
    ...
    "  log \"info\" \"First install attempt output: $install_out\"",
    "  ",
    ...
    "  else",
    "    # Check if it failed due to signatures or updates",
    "    if printf '%s\\n' \"$install_out\" | grep -qE \"(INCOMPATIBLE|SIGNATURE|UPDATE|EXISTING|SHARED_USER)\"; then",
    "      log \"warn\" \"$label: Signature/update/shared user conflict detected. Uninstalling conflicting package...\"",
    "      local uninstall_out",
    "      uninstall_out=\"$(pm uninstall \"$pkg\" 2>&1)\"",
    "      log \"info\" \"Uninstall output: $uninstall_out\"",
    "      ",
    "      # Try installing again",
    "      install_out=\"$(pm install -g -r -d \"$patched_apk\" 2>&1)\"",
    "      log \"info\" \"Second install attempt output: $install_out\"",
      "      if printf '%s\\n' \"$install_out\" | grep -iq \"Success\"; then",
    "        log \"ok\" \"$label: pm install succeeded after uninstalling conflict.\"",
    "        cmd package set-installer \"$pkg\" com.android.shell >/dev/null 2>&1 || true",
    "        pm set-installer \"$pkg\" com.android.shell >/dev/null 2>&1 || true",
    "        return 0",
    "      fi",
    "    fi",
    "    ",
    "    log \"warn\" \"$label: pm install failed! Final Output: $install_out\"",
    ...
    "",
    "# === Background storage monitor and logcat copy ===",
    "(",
    "  # Wait for decrypted storage availability (up to 5 mins)",
    "  limit=150",
    "  i=0",
    "  storage_ok=0",
    "  storage_base=\"\"",
    "  while [ $i -lt $limit ]; do",
    "    storage_base=\"$(find_download_dir)\"",
    "    if [ -n \"$storage_base\" ] && is_storage_writable \"$storage_base\"; then",
    "      storage_ok=1",
    "      break",
    "    fi",
    "    sleep 2",
    ...
    "    while IFS='|' read -r pkg label fallback_path; do",
    "      [ -n \"$pkg\" ] || continue",
    "      dl_dir=\"$storage_base/${label} Log\"",
    "      mkdir -p \"$dl_dir\" 2>/dev/null || continue",
    "      cp -f \"$LOG\" \"$dl_dir/module.log\" 2>/dev/null || true",
    "      DL_LOG_DIR=\"$dl_dir\"",
    "      log \"ok\" \"Log file successfully copied to decrypted storage: $dl_dir/module.log\"",
    "      log \"ok\" \"Starting logcat monitor for $label -> $dl_dir/logcat.log\"",
    "      ( monitor_app_logcat \"$pkg\" \"$label\" \"$dl_dir\" ) &",
    "    done <<EOF_STORAGE_APPS",
    "$APP_LIST",
    "EOF_STORAGE_APPS",
    "  else",
    "    log \"warn\" \"Decrypted storage never became available within 5 mins. Logs could not be copied to SD card.\"",
    "  fi",
    ") &",
    "",
  ].join("\n");
